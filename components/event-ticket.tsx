...
export function EventTicket({ event }: EventTicketProps) {
  ...
  const getFormattedDate = () => {
    const startStr = new Intl.DateTimeFormat("en-US", formatOptions).format(
      start,
    );
    if (end && start.getDate() !== end.getDate()) {
      const endStr = new Intl.DateTimeFormat("en-US", formatOptions).format(
        end,
      );
  ...
  const getFormattedTime = () => {
    ...
    const startStr = new Intl.DateTimeFormat("en-US", timeOptions).format(
      start,
    );
    if (end) {
    ...
    >
      ...
      >
        ...
        {countdownText ? (
          <>
            <Timer
              className={`h-8 w-8 mx-auto mb-4 ${isLive ? "text-sunset-orange" : "text-sunset-orange"}`}
            />
            <p
              className={`text-4xl md:text-5xl font-bold font-display tracking-tight ${isLive ? "text-foreground" : "text-foreground"}`}
            >
              {countdownText}
            </p>
            <p
              className={`text-sm md:text-base mt-2 ${isLive ? "text-muted-foreground" : "text-muted-foreground"}`}
            >
              {countdownLabel}
            </p>
          </>
          ...
          <>
            <div
              className={`h-4 w-4 rounded-full mx-auto mb-4 animate-pulse ${isLive ? "bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.6)]" : "bg-muted"}`}
            />
            <p
              className={`text-xl font-bold font-display ${isLive ? "text-foreground" : "text-foreground"}`}
            >
              {countdownLabel}
            </p>
            {isLive && (
              <Link
                ...
                className="mt-6 px-6 py-2.5 text-white font-semibold rounded-full bg-sunset-orange hover:bg-sunset-orange/80 transition-colors text-sm shadow-md flex items-center gap-2"
              >
                Watch Live Session
      ...
      >
        <div>
          ...
          <p className="font-semibold text-base min-[1920px]:text-lg text-foreground">
            {formattedDate}
          </p>
        </div>
